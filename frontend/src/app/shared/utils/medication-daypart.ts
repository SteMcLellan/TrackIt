 ...
 * Windows are product-defined and intentionally UI-only.
 */
...
export function medicationDaypartFromDate(date: Date): MedicationDaypart {
  const hh = String(date.getHours()).padStart(2, '0');
  const mm = String(date.getMinutes()).padStart(2, '0');
  return medicationDaypartFromLocalTime(`${hh}:${mm}`) ?? 'Evening';
}
